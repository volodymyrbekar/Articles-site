...
class RecipeTestCase(TestCase):
    def setUp(self):
        self.user_a = User.objects.create_user('admin', password='abc123')
        self.recipe_a = Recipe.objects.create(name='Test Recipe', user=self.user_a)
        self.recipe_b = Recipe.objects.create(name='Test Recipe Plow', user=self.user_a)
        self.recipe_ingredient_a = RecipeIngredient.objects.create(recipe=self.recipe_a, name='Rice',
    ...
    def test_user_two_relation(self):
        user = self.user_a
        qs = RecipeIngredient.objects.filter(recipe__user=user)
        self.assertEqual(qs.count(), 1)

    def test_user_two_relation_reverse(self):
        user = self.user_a
